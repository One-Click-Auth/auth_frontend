...
import { useOrgData, useSecurityStore  } from '../login/widgetStore';
    ...
    return (
      <Card className="border-none shadow-none flex flex-col sm:items-start ">
        ...
        <CardContent className="space-y-10 w-full">
          ...
          <div className= {`gap-2 flex flex-col items-start ${password?'cursor-not-allowed opacity-50':''} `}>
              ...
              <div>
                ...
                <div className="w-full sm:max-w-[450px] sm:min-w-[400px]">
                  <p className="text-left text-muted-foreground">
                    {password?'A password is already set for this account.':`
                     No password is currently set for this account, Set a new
                     password (optional).
                    `}
                   
                  </p>
                  <div className={`${!showChecks?'h-0 transition-all overflow-hidden':'h-[200px] transition-all '}`}>
                  <PasswordCheck pass={pass} />
                  </div>
                  
                </div>
              </div>
              <Button variant={'black'} className="w-full sm:w-[140px] sm:min-w-[140px]" disabled={password}>
                Save Update
              </Button>
            </div>
          </div>
          ...
          <div className="gap-2 flex flex-col sm:items-start ">
             ...
             <div className="flex flex-col sm:items-center flex-wrap sm:flex-row gap-4 w-full">
              ...
              {showMfa?<>
                <p className="text-left text-muted-foreground">
                {showQr?'Scan the QR and Enter the OTP from your Authentication App to Turn on MFA':'Enter OTP from your Authentication App to Turn off MFA'}   
              </p>
              </>:<>
              <p className="text-left text-muted-foreground">
                {mfa?'Multi Factor authentication (MFA) is enabled':'Multi Factor authentication (MFA) is disabled'}   
              </p>
              {mfa?
                <Button variant={'black'} className="w-full sm:w-[140px] sm:min-w-[140px]" onClick={disableMfa}>
                Disable MFA
              </Button>
                :
              <Button variant={'black'} className="w-full sm:w-[140px] sm:min-w-[140px]" onClick={registerMfa}>
                Enable MFA
              </Button>
               }
              </>}
            ...
            <div className={` ${!showMfa?'h-0 overflow-hidden':(showQr?'h-[240px] sm:h-[160px]':'h-[`100px] mt-6')} flex flex-col-reverse flecx-wrap sm:flex-row items-center w-full  p-0 gap-4 sm:gap-20 sm:mt-4`}>
             <div>
             <OTPInput
